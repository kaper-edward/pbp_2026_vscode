...
| **Moodle VPL Practice 0.7.4** | VS Code의 Moodle 명령 사용 | 저장소의 [VSIX](tools/moodle-vpl-practice-0.7.4.vsix)를 별도 설치 |
...
Extensions 화면에서 `Moodle VPL Practice`가 **0.7.4**인지 확인하거나 아래 명령의 목록에서 `leafmill.moodle-vpl-practice@0.7.4`을 찾습니다.

```text
code --list-extensions --show-versions
```

설치 파일 확인이 필요하면 저장소 루트에서 PowerShell의 `Get-FileHash tools/moodle-vpl-practice-0.7.4.vsix -Algorithm SHA256`을 실행하여 [SHA256SUMS.txt](tools/SHA256SUMS.txt)와 비교합니다. Linux에서는 `(cd tools && sha256sum -c SHA256SUMS.txt)`, macOS에서는 `(cd tools && shasum -a 256 -c SHA256SUMS.txt)`를 사용할 수 있습니다.
...
| 이전 명령이나 화면이 계속 표시됨 | 설치 버전 0.7.4 확인 후 Reload Window. 저장소 갱신만으로 설치된 확장은 바뀌지 않음 |
...
3. VSIX 0.7.4 설치 상태를 확인하고 세 실습의 `testClasses`를 실행합니다. GUI를 사용할 수 있으면 프로젝트 JDK·실행 구성·Moodle 명령도 확인합니다.
