...
| **Moodle VPL Practice 0.6.3** | VS Code의 Moodle 명령 사용 | 저장소의 [VSIX](tools/moodle-vpl-practice-0.6.3.vsix)를 별도 설치 |
...
code --install-extension tools/moodle-vpl-practice-0.6.3.vsix
```

`code`가 없으면 GUI로 설치할 수 있습니다. Extensions에서 Microsoft의 **Extension Pack for Java**와 **Gradle for Java**를 검색하고 설치합니다. Moodle 확장은 **F1 → Extensions: Install from VSIX...** 또는 **Extensions → … → Install from VSIX...**에서 `tools/moodle-vpl-practice-0.6.3.vsix`를 선택합니다. 저장소의 추천 확장 목록이나 Git clone만으로 Moodle VSIX가 자동 설치되지는 않습니다. [VS Code 확장 설치 안내](https://code.visualstudio.com/docs/configure/extensions/extension-marketplace)
...
Extensions 화면에서 `Moodle VPL Practice`가 **0.6.3**인지 확인하거나 아래 명령의 목록에서 `leafmill.moodle-vpl-practice@0.6.3`을 찾습니다.

```text
code --list-extensions --show-versions
```

설치 파일 확인이 필요하면 저장소 루트에서 PowerShell의 `Get-FileHash tools/moodle-vpl-practice-0.6.3.vsix -Algorithm SHA256`을 실행하여 [SHA256SUMS.txt](tools/SHA256SUMS.txt)와 비교합니다. Linux에서는 `(cd tools && sha256sum -c SHA256SUMS.txt)`, macOS에서는 `(cd tools && shasum -a 256 -c SHA256SUMS.txt)`를 사용할 수 있습니다.
...
F1의 Moodle 핵심 명령은 **Download Practices, Run Public Tests, Submit and Run on Moodle, Submit and Grade, Login, Logout**입니다. 0.6.3에서는 **Open Practice가 제거**되었습니다. **Download Practices**는 자료를 내려받고 폴더 경로를 안내하며 새 VS Code 창을 열지 않습니다. 추가 실습은 필요할 때만 다운로드한 뒤 File → Open Folder로 직접 엽니다. 문제 설명은 Explorer의 **Moodle VPL Practices**에서 실습 항목을 클릭하여 확인합니다.
...
| `Open Practice`가 계속 표시됨 | 설치 버전 0.6.3 확인 후 Reload Window. 저장소 갱신만으로 설치된 확장은 바뀌지 않음 |
...
3. VSIX 0.6.3 설치 상태를 확인하고 세 실습의 `testClasses`를 실행합니다. GUI를 사용할 수 있으면 프로젝트 JDK·실행 구성·Moodle 명령도 확인합니다.
